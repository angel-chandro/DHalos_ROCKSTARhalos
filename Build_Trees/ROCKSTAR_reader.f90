...
contains
  ...
  subroutine read_ROCKSTAR(ROCKSTAR_path, isnap, ilast, subhalo, mpart, lbox)
    ...
    integer(kind=int8byte) :: inhalo,hhalo,id,did,id_inhalo,id_hhalo,idhalo
    ...
    do while (ios.eq.0)
       ...
       subhalo(isub)%fof_index   = int(id_inhalo,4)

       ! Position of subhalo in input catalogue
       subhalo(isub)%index       = int(id_hhalo,4)
       ...
       if(idhalo.lt.1e9) then
          did = idhalo + (nsnap+1)*IDCorr ! special format with IDCorr
       else
          did = idhalo
       end if
       subhalo(isub)%descendant_id = did
    ...
    print*,maxval(subhalo(1:nsub)%descendant_snapnum)
    close(20)
